...
# setdefault(key, default=None)：如果键 key 不存在于 dict 中，则将 key 和 default 添加到 dict 中，并返回 default；如果键 key 已经存在，则返回对应的值。
...
my_dict[
    "e"
] += 5  # 如果键 "e" 不存在，则会引发 KeyError，因为无法对不存在的键进行加法操作。
